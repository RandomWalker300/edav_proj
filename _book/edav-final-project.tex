...
\CommentTok{# Read in data set so each line is a character string}
...
\KeywordTok{library}\NormalTok{(stringr)}
\NormalTok{header_locations <-}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(storm_strings))[}\KeywordTok{str_count}\NormalTok{(storm_strings, }\StringTok{"}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{,"}\NormalTok{) }\OperatorTok{==}\StringTok{ }\DecValTok{3}\NormalTok{]}

\CommentTok{# Extract length of each sub-dataset}
\NormalTok{headers <-}\StringTok{ }\KeywordTok{as.list}\NormalTok{(storm_strings[header_locations])}
\NormalTok{headers_df <-}\StringTok{ }\NormalTok{headers }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{map}\NormalTok{(str_sub, }\DataTypeTok{start =} \DecValTok{1}\NormalTok{, }\DataTypeTok{end =} \DecValTok{-2}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# to remove trailing comma}
\StringTok{  }\KeywordTok{map}\NormalTok{(paste0, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{) }\OperatorTok{%>%}\StringTok{                 }\CommentTok{# to trigger literal read}
\StringTok{  }\KeywordTok{map_df}\NormalTok{(read_csv, }\DataTypeTok{col_names =} \KeywordTok{c}\NormalTok{(}\StringTok{"id"}\NormalTok{, }\StringTok{"name"}\NormalTok{, }\StringTok{"n_obs"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{name =} \KeywordTok{recode}\NormalTok{(name, }\StringTok{"UNNAMED"}\NormalTok{ =}\StringTok{ }\NormalTok{id), }\DataTypeTok{skip =}\NormalTok{ header_locations) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(name, skip, n_obs)}
...
\KeywordTok{names}\NormalTok{(storm_dfs) <-}\StringTok{ }\NormalTok{headers_df}\OperatorTok{$}\NormalTok{name}
\end{Highlighting}
\end{Shaded}
